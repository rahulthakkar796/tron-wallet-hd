...
KeyStore.prototype.init = function (mnemonic, pwDerivedKey, salt) {
    let hdPathString=`m/44'/195'/0/0`;
  this.salt = salt;
...
KeyStore.prototype.generateNewAddress = async function (pwDerivedKey, n) {
  Assert.derivedKey(this, pwDerivedKey);
  ...
  const keys = await this._generatePrivKeys(pwDerivedKey, n);
  
  
  for (let i = 0; i < n; i++) {
    const keyObj = keys[i];
  
    const address= await TronWeb.address.fromPrivateKey(keyObj.privKey);
    let isValid= await utils.validateAddress(address);
    ...
    // this.encPrivKeys[address] = keyObj.encPrivKey;
    // console.log("pk:",    this.encPrivKeys[address])
    // this.addresses.push(address);
...
KeyStore.prototype._generatePrivKeys = async function (pwDerivedKey, n) {
  Assert.derivedKey(this, pwDerivedKey);
...
KeyStore.createVault = function (opts, cb) {
  const {seedPhrase, password } = opts;
  let salt = opts.salt;
...
KeyStore.DEFAULT_SALT = 'tronhdwallet';
